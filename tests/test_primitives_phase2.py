...
def test_window_flag_numeric_field_flags_burst(registry):
    ...
    rows = {(r["t"], r["ip"]): r for r in registry.materialize(out.handle.handle)}
...
def test_window_flag_iso8601_string_field(registry):
    ...
    rows = {r["ts"]: r for r in registry.materialize(out.handle.handle)}
...
def test_window_flag_wrong_handle_kind_raises(registry):
...
def _org(registry, records):
    ...
    out = call_primitive(
        registry,
        "index_by",
        {"handle": nodes.handle, "key_fields": ["id"], "name": _fixture_name(), "description": "d"},
    )
...
def test_graph_walk_resolve_deny_overrides_inherited_allow(registry):
    ...
    assert out.status == "ok"
    rows = {r["node_id"]: r["effect"] for r in registry.materialize(out.handle.handle)}
...
def test_graph_walk_resolve_wrong_handle_kinds_raise(registry):
    ...
    with pytest.raises(TypeMismatchError):
        call_primitive(
            ...
            "graph_walk_resolve",
            {
                "nodes_index_handle": nodes.handle,
                "parent_field": "manager_id",
                "rules_handle": rules.handle,
                "rule_node_field": "employee_id",
                "group_field": "resource",
